...
// Default only for non-value types (void). Any new value type MUST inherit
// ValueIpcType<T> (or override copyStorage) so property reads cannot silently
// store a null slot the way a forgotten override would with this stub.
void* IpcType::copyStorage(const void* /*data*/) const { return nullptr; }
